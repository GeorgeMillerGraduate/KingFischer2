...
public final class GameModel {
  ...
  public void play(int move) {
    if (result() != null) throw new IllegalStateException("Game has ended");
  ...
  public Position at(int ply) {
    if (ply < 0 || ply > size()) throw new IllegalArgumentException("Invalid history position");
    Position p = new Position();
    for (int i = 0; i < ply; i++) p.make(moves.get(i));
    return p;
  }
  ...
  public String result() {
    ...
    if (legal().length == 0)
      return position.inCheck()
          ...
          : "Draw by stalemate";
    if (position.halfmove >= 100) return "Draw by fifty-move rule";
    if (position.repetition(0)) return "Draw by threefold repetition";
