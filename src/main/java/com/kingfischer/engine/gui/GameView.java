...
public final class GameView extends BorderPane implements AutoCloseable {
  ...
  public GameView(SetupDialog.Settings settings, Consumer<SetupDialog.Settings> newGame) {
    ...
    setOnKeyPressed(
        e -> {
          if (e.getTarget() instanceof ComboBoxBase<?>) return;
  ...
  private void clicked(int square) {
    if (thinking
        ...
        || viewPly >= 0
        || game.result() != null
        || game.position().side != settings.human()) return;
    if (selected >= 0) {
      int[] choices =
      ...
      if (choices.length > 0) {
        ...
        if (choices.length > 1) {
          ...
          for (int m : choices) if (Move.promotion(m) == type) move = m;
        }
        play(move);
        schedule();
        return;
  ...
  private void play(int move) {
    ...
    selected = -1;
    refresh();
    if (viewPly < 0) board.animate(move, piece);
  }
  ...
  private void refresh() {
    ...
    for (int i = 0; i < game.size(); i += 2) {
      ...
      for (int j = i; j < Math.min(i + 2, game.size()); j++) {
        ...
        if (target == ply) b.getStyleClass().add("current-move");
        row.getChildren().add(b);
      }
      moves.getChildren().add(row);
    }
    if (viewPly < 0) javafx.application.Platform.runLater(() -> moveScroll.setVvalue(1));
  }
