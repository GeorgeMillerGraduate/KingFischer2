...
public final class MoveNotation {
  ...
  public static String format(Position p, int move, boolean doubleCheck) {
    ...
    if ((move & Move.CASTLE) != 0) s.append(to > from ? "O-O" : "O-O-O");
    else {
      ...
      if (type != 1) {
        ...
        if (other) {
          if (!sameFile) s.append((char) ('a' + (from & 7)));
          else if (!sameRank) s.append((char) ('1' + (from >>> 3)));
          else s.append(Move.square(from));
        }
      } else if (capture) s.append((char) ('a' + (from & 7)));
      if (capture) s.append('x');
      s.append(Move.square(to));
      if (Move.promotion(move) != 0) s.append('=').append(" PNBRQK".charAt(Move.promotion(move)));
    }
    p.make(move);
    try {
      if (p.inCheck()) {
        if (MoveGenerator.legal(p, new int[256]) == 0) s.append('#');
        else if (doubleCheck
