...
/** Searches snapshots off the event thread; generations reject cancelled results. */
    ...
    worker.submit(
        () -> {
          try {
            ...
            if (network == null) network = Network.bundled();
            if (generation.get() != ticket) return;
            Search s =
                new Search(
                    snapshot,
            ...
            if (ThreadLocalRandom.current().nextDouble() < 0.8 * (1 - x) * (1 - x)) {
              ...
              if (n > 0) move = legal[ThreadLocalRandom.current().nextInt(n)];
            }
            final int chosen = move;
            Platform.runLater(
                () -> {
                  if (generation.get() == ticket) complete.accept(chosen);
                });
          } catch (Exception ex) {
            Platform.runLater(
                () -> {
